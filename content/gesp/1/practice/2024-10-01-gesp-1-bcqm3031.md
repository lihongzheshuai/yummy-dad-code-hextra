---
layout: post
title: BCQM3031 浮点计算，国庆快乐
date: 2024-10-01T20:00:00+0800
author: OneCoder
comments: true
tags:
- GESP
- C++
- 基础语句
categories:
- GESP
- 一级
slug: gesp-1-bcqm3031
type: docs
weight: 23
math: true
image: /images/post/gesp/gesp-1-v2.png
---
浮点数计算+数位保留练习，%m.nf知识点，已在[BCQM3027](https://www.coderli.com/gesp-1-bcqm3027/)中详细介绍。

<!--more-->

## BCQM3031 浮点计算，国庆快乐

### 题目要求

#### 描述

>已知小明有n元，他买了一本书，这本书原价为m元，现在打8折出售。求小明还剩多少钱(保留2位小数)。

#### 输入

>输入n,m。

#### 输出

>小明还剩多少钱(保留2位小数)。

#### 输入样例

>100 100

#### 输出样例

```console
20.00
```

---

### 题目分析

没想到这道题对我孩子的难点居然是不知道什么是8折，小朋友确实没自己买过什么东西。8折就是：*价格 * 0.8*。知道这个关系剩下就是简单的计算和按格式输出了。浮点数格式化***%m.nf***知识点，已在[BCQM3027](https://www.coderli.com/gesp-1-bcqm3027/)中详细介绍，不再赘述。

---

### 示例代码

```cpp
#include <cstdio>
int main() {
    int n, m; // 定义整型变量n和m，分别表示小明原有的钱数和书的原价
    scanf("%d%d", &n, &m); // 从标准输入读取两个整数，赋值给n和m
    printf("%.2lf", n - m * 0.8); // 计算剩余的钱数：原有钱数 - 打折后的书价(保留两位小数)
    return 0; // 程序正常退出
}
```

---
