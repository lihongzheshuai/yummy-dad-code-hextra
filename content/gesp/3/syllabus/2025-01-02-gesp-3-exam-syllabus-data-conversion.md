...
- GESP
- 三级
slug: gesp-3-exam-syllabus-data-conversion
type: docs
weight: 2
math: true
mermaid: true
...
- **特点：**
  - 是计算机系统的基础语言。
  - 易于通过物理电路（高低电平）实现。
- **权值分布：** 从右到左，每一位的权值为 $(2^n)$ ($n$从$0$开始)。
...
- **权值分布：** 从右到左，每一位的权值为 $(8^n)$ ($n$从$0$开始)。
...
- **权值分布：** 从右到左，每一位的权值为 $(16^n)$ ($n$从$0$开始)。
